...
Long-horizon autonomous runs where drift is costly, genuinely novel work with no known approach, and final gates on decisions that are expensive *and irreversible*. The test is whether the extra capability changes the **outcome**, not merely the polish — if an Opus retry would plausibly get there, use Opus. At 2x Opus in both directions, this tier earns its cost only rarely. Log every Fable call; if one wouldn't have changed the decision versus Opus, that's a routing bug — tighten the signals.
...
4. **Swarm fan-out control.** Planner/orchestrator = Opus or Sonnet. Repetitive worker subagents = Haiku. Never pay Opus rates for parallel grunts.
